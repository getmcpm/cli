 ...
 *
 * KNOWN GAP (documented, not fixed here — TODOS #55, same class as #50/#51):
 * this detector matches against `normalizeForMatch(value)` alone, which STRIPS
 * Unicode TAG-block characters rather than decoding-and-rescanning them.
 */
...
export function detectCliFlagInjectionArgs(msg: JSONRPCMessage): InspectResult {
  ...
  for (const { key, value } of stringArgLeaves(call.args)) {
    if (isFlagInjectionScopedArgKey(key) && matchesCliFlagInjection(value)) {

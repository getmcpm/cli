...
| Unicode TAG block | Payload concealed in U+E0000–U+E007F on any carrier ("ASCII smuggling") | decoded and re-scanned — the recovered signature decides, with a known bypass on the injection family (below) |
...
> ⚠ **Known bypass in the TAG decode pass, unreleased and unfixed.** A visible decoy phrase can suppress the concealed finding for the **instruction-injection** signatures, dropping a `block` to a `warn`. The suppression that stops the decoded pass double-reporting text you can already read is keyed on the *rendered text* of a match, and an attacker writes both sides of that comparison: any phrase preceded by a character outside `[\s.,;:!?]` — `'ignore all previous instructions'` in quotes, say — is not reported itself but cancels the real payload. The **credential-phishing** family is unaffected (its patterns carry no leading anchor, so nothing new can be suppressed), which means the TAG-encoded wallet-seed case above still blocks. Tracked as TODOS #34 and fixed before this ships — the class has never appeared in a published release.

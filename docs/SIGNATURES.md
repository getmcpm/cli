...
>
> ⚠ **Known bypass, unreleased and unfixed (TODOS #34).** Pass (1) suppresses a decoded finding that is already visible in plain sight, so that an article *quoting* an attack phrase is not blocked for quoting it. That suppression is keyed on the **rendered text** of a match, and both sides of the comparison are attacker-written: a visible phrase preceded by any character outside `[\s.,;:!?]` matches the relaxed pattern without being reported itself, and cancels the concealed payload — `block` becomes `warn`, leaving only the presence floor. It affects the five signatures whose first pattern carries the `(?:^|[\s.,;:!?])` leading anchor (the `owasp-mcp-{1,2}` injection family, on every carrier). It does **not** affect `credential-phishing-*`: those patterns have no leading anchor, so relaxation is the identity and the suppression set can only contain findings the strict scan already reported — which is why the `sampling_prompt` wallet-seed case in the paragraph above still holds. The fix replaces text-identity with an occurrence count against a masked copy of the same segment, so a payload whose literals come from concealed content always survives as surplus. Not yet implemented; it gates tagging v0.28.0.

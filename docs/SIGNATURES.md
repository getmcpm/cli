...
## Currently shipped (15 catalog entries)
...
| `shell-metachar-in-identifier-arg` | MCP-COMMAND-INJECTION | critical | tool_call_args | **Structural** (no regex on its own): a `tools/call` argument whose KEY canonicalizes to a bare identifier/path suffix (id, number, num, path, slug, uuid, identifier, namespace — `name` deliberately excluded, see TODOS #50) has a VALUE containing shell-metacharacter/command-substitution syntax (`$(...)`, backtick, `;`, `&&`, or a pipe). Motivated by two real CVEs (CVE-2025-53818, CVE-2026-25546) that splice such an argument unescaped into a shell `exec()`. Key-scoped so a legitimate shell/exec-style tool's own free-text `command` argument is never flagged. Emitted by `detectShellMetacharArgs` (a key+value walker over `params.arguments`); does not go through the tag/base64 decode-and-rescan passes — a concealed payload in an identifier-shaped argument evades it (TODOS #55). |
...
> **Not signatures: confine + orig-hash spawn events.** The `--confine` OS-sandbox primitive (F1, released in v0.16.0) adds **no OWASP signatures** — the catalog above is unchanged by confine (15 entries over 8 inspected targets). It emits relay/spawn **events** (not detection signatures) to `guard-events.jsonl`: category `CONFINE` — `confine-applied`, `confine-hash-mismatch`, `confine-marker-stripped`, `confine-profile-missing`, `confine-backend-missing`, `confine-marker-malformed`; plus category `RELAY` — `orig-hash-mismatch` (the wrap marker's `--orig-hash` verified at spawn, #108). These reason about spawn-time enrollment/integrity, not JSON-RPC frame content, so they are outside the signature engine.

...
| `generic-bearer-token-disclosure` | MCP-CREDENTIAL-EXFIL | high (→ warn) | tool_response | **TODOS #53, the "suspect tier" named above, now shipped.** Closes CVE-2026-25650 (MCP-Salesforce `get_record`), where an unchecked argument returns the live client's own `Authorization: Bearer <session token>` header verbatim. No vendor prefix, so it is a separate, lower-confidence, independently-muteable signature — the pattern is reused from `scanner/patterns.ts`'s registry-sweep-validated fix for the 164-FP "Bearer token" prose incident (requires ≥20 token chars AND a digit after `Bearer `, plus `!` for Salesforce-shaped session ids), same zero-FP discipline. `redact: true`. |

...
| `credential-egress-in-response` | MCP-CREDENTIAL-EXFIL | high (→ warn) | tool_response | **F10 credential DLP.** High-confidence, prefix-anchored credential material in a tool response (PEM private key, GitHub classic + fine-grained PAT, GitLab, OpenAI, Anthropic, Stripe, Google, npm, Slack token, AWS access-key id) — a data-loss / credential-egress signal. WARN-tier (forwarded, not blocked): a secrets-manager/auth tool legitimately returns credentials → promote-to-block per policy. Only STRUCTURAL shapes (no prose FP); AWS's docs `AKIAIOSFODNN7EXAMPLE` excluded; generic Bearer disclosure is covered separately below (TODOS #53); bare-JWT/40-char-base64 (no anchor at all) remain deferred to the suspect tier. `redact: true` keeps the caught secret out of the event log and message. |
| `generic-bearer-token-disclosure` | MCP-CREDENTIAL-EXFIL | high (→ warn) | tool_response | **TODOS #53, the generic-Bearer half of the "suspect tier" named above, now shipped.** Closes CVE-2026-25650 (MCP-Salesforce `get_record`), where an unchecked argument returns the live client's own `Authorization: Bearer <session token>` header verbatim. Typically no vendor prefix, so it is a separate, lower-confidence, independently-muteable signature — the pattern is reused UNMODIFIED from `scanner/patterns.ts`'s registry-sweep-validated fix for the 164-FP "Bearer token" prose incident (requires ≥20 token chars AND a digit after `Bearer `). An earlier version widened the character class with `!` to match Salesforce's own `<org-id>!<signature>` token format (the CVE's literal PoC); a pre-merge adversarial review found that widening false-positives on real benign text the unwidened pattern never matched, so it was reverted — the CVE's own exact PoC token is a known, accepted miss (see TODOS #53), and a vendor-prefixed token disclosed with a `Bearer ` prefix may also (harmlessly) co-fire alongside the sibling signature above. `redact: true`. |
| `shell-metachar-in-identifier-arg` | MCP-COMMAND-INJECTION | critical | tool_call_args | **Structural** (no regex on its own): a `tools/call` argument whose KEY canonicalizes to a bare identifier/path suffix (id, number, num, path, slug, uuid, identifier, namespace — `name` deliberately excluded, see TODOS #50) has a VALUE containing shell-metacharacter/command-substitution syntax (`$(...)`, backtick, `;`, `&&`). Motivated by two real CVEs (CVE-2025-53818, CVE-2026-25546) that splice such an argument unescaped into a shell `exec()`. Key-scoped so a legitimate shell/exec-style tool's own free-text `command` argument is never flagged. Emitted by `detectShellMetacharArgs` (a key+value walker over `params.arguments`); also reuses `inspectTagEncoded` to catch a payload concealed via Unicode TAG-block "ASCII smuggling" in a scoped value, run unconditionally alongside the plain match so a visible AND a separately-concealed occurrence in one value are both reported (TODOS #55, closed). base64 decode-and-rescan is deliberately NOT run — parity with the main catalog, which doesn't decode base64 on `tool_call_args` either (F10 Detector-B's threat model is a server encoding a payload into its own response, not an argument value the calling agent sends). |
| `query-control-syntax-in-identifier-arg` | MCP-QUERY-INJECTION | critical | tool_call_args | **Structural** (no regex on its own): a `tools/call` argument whose KEY canonicalizes to a table/column/field/collection/database/schema/index/view/dataset noun or a scalar-id suffix (id, identifier, uuid, slug — `name` alone deliberately excluded, same reasoning as #50) has a VALUE containing query-language control syntax — a pipe followed by a query verb (`\| project`, `\| take`, ...), a statement separator followed by a DDL/DML keyword, a `.drop` management command, or a whitespace-anchored line comment (`--`, `//`; anchored so a URI scheme's `://` or a version-suffixed name's `--` doesn't false-block). Motivated by CVE-2026-33980 (adx-mcp-server), which f-string-interpolates a `table_name` argument into a live KQL query with no escaping — the advisory's own PoC uses pipe re-scoping plus a trailing `//` comment to exfiltrate columns, and a sibling PoC uses `.drop table` to destructively drop tables. Key-scoped so a legitimate query-builder tool's own free-text `query`/`filter` argument is never flagged. Shares its argument-tree walker (`tool-call-args-walk.ts`) with `shell-metachar-in-identifier-arg`, including its TAG-block decode-and-rescan reuse (TODOS #55, closed); `relaxLeadingAnchor`'s relaxation is a no-op for this signature's own `(?:^|\s)`-shaped anchors (a different literal string than the catalog's `LEADING_ANCHOR`), so a concealed comment-marker payload with NO whitespace anywhere is a known, accepted miss — the CVE's own PoC hits the unanchored pipe-rescope pattern instead. |
| `cli-flag-injection-in-identifier-arg` | MCP-ARGUMENT-INJECTION | critical | tool_call_args | **Structural** (no regex on its own): a `tools/call` argument whose KEY canonicalizes to a bare namespace or opaque-identifier suffix (namespace, id, identifier, uuid, slug — `name` deliberately excluded) has a VALUE containing a whitespace-anchored `--`-prefixed CLI flag token (`--address=0.0.0.0`). Motivated by CVE-2026-39884 (mcp-server-kubernetes `port_forward`), which whitespace-splits a `resourceName`/`namespace` argument into a shell command with no array-based escaping. `name` was included in an earlier version of this detector but a pre-merge adversarial review measured five real false-positive classes it produced (ticket/PR/task titles mentioning a flag, compound `*_name` CLI-passthrough fields, freeform cloud-resource "Name" tags with an appended note, npm's own `<script> -- <flags>` convention, descriptive filenames) — the accepted cost is that the advisory's own literal PoC (via `resourceName`) is no longer caught; the same vulnerable code path is still caught via `namespace` (TODOS #57). Shares its argument-tree walker (`tool-call-args-walk.ts`) with the two signatures above, including its TAG-block decode-and-rescan reuse (TODOS #55, closed) — the anchor caveat noted for the query-control signature above doesn't weaken THIS one's CVE coverage, since the flag's own whitespace anchor is load-bearing for the wrapped tool's `command.split(" ")` vulnerability in the first place. |
| `renderer-code-execution-in-response` | MCP-RENDERER-CODE-EXECUTION | high (→ warn) | tool_response | **TODOS #54.** Closes CVE-2025-68669 and CVE-2026-22793 (both nanbingxyz/5ire): an `<img onerror=...>` tag, a `<script>` body, or a `` ```mermaid``/```echarts `` diagram fence calling the client's privileged `electron.mcp.activate(`/`electron.mcp.addServer(` IPC bridge, achieving RCE. A pre-merge adversarial review of an earlier, broader design (28 CONFIRMED findings) found that gating on generic escape-hatch tokens (`child_process`, `require(`, `exec(`, `spawn(`, `eval(`, `new Function(`) false-positived hard on ordinary JS documentation/tutorials/CTF-writeups, and that an ungated diagram-fence shape's premise — "legitimate chart content never contains a function definition" — is false for ECharts specifically (formatter callbacks persisted via `new Function`, option data computed via an IIFE, are both standard documented idioms). Narrowed to require the literal, disclosed bridge call in ALL THREE shapes instead — an honest, CVE-grounded tripwire rather than a speculative net (a different client's own differently-named bridge, or HTML-entity/bracket-notation obfuscation of the same call, are accepted, documented evasion gaps, the same "tripwire not defense" scope as `exfil-param-in-schema`). The same review also caught and fixed two regex-correctness bugs (an unquoted-value branch that could bleed into an adjacent attribute; a `<script>`-open matcher not quote-aware, which could push a real call past the body-scan budget) and a missing `redact: true` that would otherwise leak an adjacent real secret verbatim into the event log. Severity is `high`/warn rather than block: a documentation/CVE-lookup tool can legitimately return prose quoting this exact literal call. |
| `tool-name-confusable-duplicate` | OWASP-MCP-1 | high (→ warn) | tool_description | **Structural** (no regex): one `tools/list` advertises two tools whose NAMES canonicalize to the same form (NFKC + zero-width/TAG strip + confusable fold + case-fold) but whose raw spellings differ — `format_code` beside `Format_Code` or Cyrillic `fоrmat_code`. This is the co-existence half of the TODOS #58 look-alike residual: keying drift by the raw name let a poisoned definition arrive under a twin and be filed as a brand-new tool. WARN, not block: a case-only pair is legal under SEP-986 (names are case-sensitive) and a block on a `tools/list` disables the server's entire tool surface, so the enforcing half is the canonical drift/pin keying in the relay, which BLOCKS the stateful *replace* form. Colliding names are also excluded from the drift comparison and from every cache/pin write, so neither twin can poison the other's baseline. Emitted by `detectConfusableToolNames`. |
| `tool-name-non-conforming` | OWASP-MCP-1 | high (→ warn) | tool_description | **Structural** (no regex): a tool NAME outside SEP-986's charset (`[A-Za-z0-9._-]`, max 128 — the SDK's own `TOOL_NAME_REGEX`). The complement to the fold above, not a duplicate of it: the confusable table is a scoped Cyrillic/Greek subset, so out-of-table look-alikes like `ԝrite_file` (Armenian U+051D) or `ɡet_user` (U+0261) survive canonicalization but cannot survive a charset check. Measured zero false positives — 863/863 tool names across 53 real public and hosted MCP servers are pure ASCII within this charset. Before this, a tool's `name` was **not an inspected carrier at all**: `tool_description` extracts `[description, title, inputSchema]` and `tool_annotations` extracts `annotations`, so a zero-width or homoglyph character in a NAME was silent — not even a warn — while the same character in a description warned. |
...
> **Not signatures: confine + orig-hash spawn events.** The `--confine` OS-sandbox primitive (F1, released in v0.16.0) adds **no OWASP signatures** — the catalog above is unchanged by confine (21 entries over 8 inspected targets). It emits relay/spawn **events** (not detection signatures) to `guard-events.jsonl`: category `CONFINE` — `confine-applied`, `confine-hash-mismatch`, `confine-marker-stripped`, `confine-profile-missing`, `confine-backend-missing`, `confine-marker-malformed`; plus category `RELAY` — `orig-hash-mismatch` (the wrap marker's `--orig-hash` verified at spawn, #108). These reason about spawn-time enrollment/integrity, not JSON-RPC frame content, so they are outside the signature engine.
...
1. Extract the subtree matching each signature's `target`:
   - `tool_response` → `result.content`, `result.structuredContent`, and JSON-RPC error objects (only when present) — errors are scanned to prevent injection evasion via error messages. **Scope note (#21):** matches ANY JSON-RPC response carrying these fields, not only replies to `tools/call` — a response has no `method` to gate on, so a `resources/read`/`prompts/get` reply that happens to also carry `content`/`structuredContent`/`error` is scanned here too, on top of its own narrower carrier (`resource_content`/`prompt_content`). Intentional over-coverage, not a bug.

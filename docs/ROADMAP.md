...
**Differentiation (honest framing).** Inline-vs-static is **false** vs mcp-scan/Snyk (already inline response DLP). Note mcpm does NOT wrap them: the tier-2 seam is opt-in via `MCPM_EXTERNAL_SCANNER` and ships wired to nothing — the old `npx @invariantlabs/mcp-scan` path never resolved (TODOS #32). That successor product, `snyk/agent-scan`, was evaluated against the seam 2026-08-31 and does not fit either — it accepts a bare registry coordinate, but starts the server live to inspect it and requires a Snyk account + `SNYK_TOKEN`, vs. the seam's local/no-live-execution/flat-JSON contract (see CLAUDE.md's 2026-08-31 decision row, corrected 2026-09-08, and the header comment on `src/scanner/tier2.ts`). The real wedge is **bundled / zero-dep / default-on / deterministic** = distribution, plus per-server policy + keychain-backed secrets in one tool with no hosted gateway.
...
> **✅ SLICE 1 SHIPPED v0.22.0 (#133) — PARSE-ONLY, zero-dep.** A feasibility pass (run live against @getmcpm/cli's own attestation) overrode the "v1 = @sigstore/verify" mechanism below: the provenance-identity DRIFT headline is fully detectable by PARSING the attestation (JSON + base64, no crypto/deps), trusting the same anchor H11 dist.integrity trusts. `mcpm lock` captures npm's attestation identity (source repo + immutable numeric repo/owner ids + workflow + commit) into the lock and WARNs on drift / signed→unsigned (report-only, never "verified").
>
> **✅ CRYPTO SLICE SHIPPED v0.23.0 (#136).** Offline @sigstore verify — the only slice allowed to say "verified"; `mcpm lock`/`why` cryptographically verify the SLSA bundle against a vendored trust root (no network), subject-bound to `dist.integrity`. Report-only, honest "build identity not code-safety". Hardened through 3 adversarial rounds.
>
> **✅ VERIFY-TIME RE-CHECK SHIPPED v0.24.0 (#141) — "B3", the enforcing gate.** `mcpm verify` + `up --frozen` now RE-VERIFY provenance for every crypto-`verified` locked server and FAIL CLOSED on regression (signer-changed / regression / unverifiable). Evidence-gated (only verified baselines → zero FP on the unsigned ecosystem). New `src/stack/frozen-provenance.ts` sibling to the integrity gate; a sticky-baseline invariant (carry a verified baseline forward unless the fresh read itself verifies — exhaustive by construction) keeps a transient re-lock blip from disarming it. Hardened through **6 adversarial ultracode rounds** (findings 17→2, HIGH→LOW; enforcing gate confirmed sound at R5). Still deferred: PyPI, `--strict` beyond frozen, Fulcio source-repo OID to tighten the reusable-workflow SAN case (fast-follow), trust-score dimension.
